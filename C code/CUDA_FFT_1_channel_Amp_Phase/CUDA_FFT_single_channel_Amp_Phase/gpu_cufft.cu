	...
	*/
	...
	if (fp != NULL) {
		for (i = 0; i < NALINES * DEPTH * NBLINES*2; i+=2)
			fprintf(fp, "%.10f, %.10f\n", h_processed_signal[i], h_processed_signal[i+1]);
		fclose(fp);
		printf("write to file success\n");
	}
	else
...
//#endif
	...
	checkCudaErrors(cudaMalloc((void **) &d_processed, (long long int)NALINES * size1 * sizeof(data_t) * 2 * DEPTH));
...
//    cudaPrintfEnd();
   ...
   // cudaEventRecord(start_mempyDH, 0);
	checkCudaErrors(cudaMemcpy(h_processed_signal, d_processed, NALINES * size1 * sizeof(data_t) * 2 * DEPTH,
 ...
 //second part
   ...
   // cudaPrintfEnd();
    ...
    //CUDA_SAFE_CALL(cudaPeekAtLastError());
    // Transfer the results back to the host
	checkCudaErrors(cudaMemcpy(h_processed_signal+(long long int)NALINES *size1 * 2 * DEPTH, d_processed, (long long int)NALINES * size2 * sizeof(data_t) * 2 * DEPTH,
                              cudaMemcpyDeviceToHost));
//third part
	checkCudaErrors(
   ...
   // cudaPrintfEnd();
	...
	//CUDA_SAFE_CALL(cudaPeekAtLastError());
	// Transfer the results back to the host
	checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * (size1+size2) * 2 * DEPTH, d_processed, (long long int)NALINES * size3 * sizeof(data_t) * 2 * DEPTH,
		cudaMemcpyDeviceToHost));
...
static __global__ void calc_amp(data_t *FFT_buffer, int num_Blines_per_FFT, int part, data_t *d_processed, int z0, int DEPTH, int NALINES) {
	...
	long long int step = (long long int)part * num_Blines_per_FFT*NALINES*DEPTH * 2;
	for (i = threadID; i < range; i += numThreads) {
		long long int k = i *2;   //
