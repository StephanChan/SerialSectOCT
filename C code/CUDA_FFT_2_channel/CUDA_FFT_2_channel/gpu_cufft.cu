...
int main(int argc, char **argv) {
    ...
    long long int length_data = (long long int)DEPTH * (long long int)3 * (long long int)NALINES * (long long int)NBLINES;
	...
	*/
	fp = fopen("D:\\191130\\191130_PSOCT_spectral\\combined_data\\1-1-1440-1250-1100.dat","rb");
	if (fp != NULL) {
	...
	single_channel_process(h_signal, h_processed_signal, 0,DEPTH,280,1100,NALINES, NBLINES, SAMPLES);
	...
	if (fp != NULL) {
		for (i = 0; i < NALINES * DEPTH * 100*3; i+=3)
			fprintf(fp, "%.10f, %.10f, %.10f\n", h_processed_signal[i], h_processed_signal[i+1], h_processed_signal[i+2]);
		fclose(fp);
		printf("write to file success\n");
	}
	else
...
void single_channel_process(uint16_t *h_signal, data_t *h_processed_signal, int z0, int DEPTH, int a, int b, int NALINES, int NBLINES, int SAMPLES) {
	...
	//IMPORTANT: change size1, size2, size3 if NBLINES is not 1100
	size1 = 400;
	size2 = 400;
	size3 = 300;
...
//#endif
	...
	checkCudaErrors(cudaMalloc((void **) &d_dataA, (long long int)NALINES * (long long int)size1 * (long long int)sizeof(uint16_t) * (long long int)SAMPLES*2));
	checkCudaErrors(cudaMalloc((void **) &d_processed, (long long int)NALINES * size1 * sizeof(data_t) * 3 * DEPTH));
 ...
 //first part of data
	...
	checkCudaErrors(
            cudaMemcpy(d_dataA, h_signal, (long long int)NALINES * size1 * sizeof(uint16_t) * SAMPLES*2, cudaMemcpyHostToDevice));
...
//    cudaPrintfInit();
    ...
    my_cufft(d_dataA, d_dispersion, d_hann, d_processed, z0, DEPTH, size1*2, NALINES);
...
//    cudaPrintfEnd();
   ...
   // cudaEventRecord(start_mempyDH, 0);
	checkCudaErrors(cudaMemcpy(h_processed_signal, d_processed, NALINES * size1 * sizeof(data_t) * 3 * DEPTH,
 ...
 //second part
	...
	//checkCudaErrors(cudaMemcpy(d_hann, hann, sizeof(data_t) * SAMPLES, cudaMemcpyHostToDevice));
    checkCudaErrors(
            cudaMemcpy(d_dataA, h_signal+(long long int)NALINES*size1*SAMPLES*2, (long long int)NALINES * size2 * sizeof(uint16_t) * SAMPLES*2, cudaMemcpyHostToDevice));
   
    //cudaEventRecord(start_kernel, 0);
    //kernel_process << < dimGrid, dimBlock >> > (d_dataA, d_dataB, d_dispersion, d_processed, z0);
    my_cufft(d_dataA, d_dispersion, d_hann, d_processed, z0, DEPTH, size2*2, NALINES);
   ...
   // cudaPrintfEnd();
    ...
    //CUDA_SAFE_CALL(cudaPeekAtLastError());
    // Transfer the results back to the host
	checkCudaErrors(cudaMemcpy(h_processed_signal+(long long int)NALINES *size1 * 3 * DEPTH, d_processed, (long long int)NALINES * size2 * sizeof(data_t) * 3 * DEPTH,
                              cudaMemcpyDeviceToHost));
//third part
	checkCudaErrors(
		cudaMemcpy(d_dataA, h_signal + (long long int)NALINES*(size1+size2)*SAMPLES *2, (long long int)NALINES * size3 * sizeof(uint16_t)  * SAMPLES*2, cudaMemcpyHostToDevice));
	
	//cudaEventRecord(start_kernel, 0);
	//kernel_process << < dimGrid, dimBlock >> > (d_dataA, d_dataB, d_dispersion, d_processed, z0);
	my_cufft(d_dataA, d_dispersion, d_hann, d_processed, z0, DEPTH, size3*2, NALINES);
   ...
   // cudaPrintfEnd();
	...
	//CUDA_SAFE_CALL(cudaPeekAtLastError());
	// Transfer the results back to the host
	checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * (size1+size2) * 3 * DEPTH, d_processed, (long long int)NALINES * size3 * sizeof(data_t) * 3 * DEPTH,
		cudaMemcpyDeviceToHost));
...
static __global__ void calc_amp(data_t *FFT_buffer, int num_Blines_per_FFT, int part, data_t *d_processed, int z0, int DEPTH, int NALINES) {
	...
	long long int i;
	long long range = (long long int)num_Blines_per_FFT / 2 * NALINES * DEPTH;
	long long int step = (long long int)part * num_Blines_per_FFT / 2 * NALINES*DEPTH * 3;
	long long int B_step = (long long int)NALINES * 4096;
	for (i = threadID; i < range; i += numThreads) {
		long long int k = i * 3;   //
		int t = int(i / (DEPTH*NALINES));
		long long int d = (long long int)t*NALINES * 2 * 4096 + (long long int)((i - (long long int)t*NALINES*DEPTH) / DEPTH) * 4096 + (i % DEPTH + z0) * 2;
		data_t a1 = (data_t)(pow(FFT_buffer[d] / 1440, 2) + pow(FFT_buffer[d + 1] / 1440, 2));
		data_t a2 = (data_t)(pow(FFT_buffer[d + B_step] / 1440, 2) + pow(FFT_buffer[d + 1 + B_step] / 1440, 2));
		data_t p1 = (data_t)atan(FFT_buffer[d + 1] / FFT_buffer[d]);
		data_t p2 = (data_t)atan(FFT_buffer[d + 1 + B_step] / FFT_buffer[d + B_step]);

		//cuPrintf("i is %d, d is %d, a is %f, FFT_buffer is%f\n", i, d, a, FFT_buffer[d]);
		d_processed[k + step] = sqrt(a1+a2);
		d_processed[k + step + 1] = atan(sqrt(a1/a2))/PI*180;
		d_processed[k + step + 2] = (p1-p2)/2/PI*180;
	}
}
