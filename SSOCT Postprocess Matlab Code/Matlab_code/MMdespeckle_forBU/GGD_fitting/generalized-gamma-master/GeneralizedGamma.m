...
% doi.org/10.1061/9780872629332.038
   ...
   methods
      ...
      function parmHat = fitDist(this, sample)
          ...
          start = [1 1 1.5];
          lb = [0.01 0.01 0.01];
          ub = [50000 200 200]; %changed by stephan to avoid endless fitting in low SNR depth
          opt=statset('MaxIter',15,'MaxFunEvals',15,'FunValCheck','on');%added by stephan to avoid non-convergence
          parmHat = mle(sample, 'pdf', @(x, lambda, c, m) ...
              this.pdf(sample, lambda, c, m), ...
              'Options',opt,'start', start, 'lower', lb, 'upper', ub);
