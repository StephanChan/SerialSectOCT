function Surf_stitch(target, datapath,disp,mosaic,pxlsize,islice,pattern,sys)
...
% filepath=strcat(datapath,'fitting/vol2/');
...
%         filename0=dir(strcat(target,'-',num2str(islice),'-',num2str(in),'.mat'));
        ...
        if strcmp(sys,'PSOCT')
            Mosaic(row,column)=Mosaic(row,column)+sur.*Masque2(row,column); %#################################change us to ub if for mub stitch 
        elseif strcmp(sys,'Thorlabs')
