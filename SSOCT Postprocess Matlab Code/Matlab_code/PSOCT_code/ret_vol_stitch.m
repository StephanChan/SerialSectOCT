...
% id=str2num(id);
filename = strcat(datapath,'aip/vol2/');
f=strcat(filename,'TileConfiguration.registered.txt');
...
for nslice=id
    ...
    Mosaic = zeros(round(max(Xcen/4))+round(Xsize/8) ,round(max(Ycen/4))+round(Ysize/8),round(thickness/1));
