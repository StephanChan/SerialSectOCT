...
%         cross(21:end-10,:,:)=tmp;
        ...
        ret_aip=single(squeeze(mean(ret(35:160,:,:),1)));

        aipname=strcat(folder,'retardance/vol',num2str(slice_index),'/',coord,'.mat');
        save(aipname,'ret_aip');
        
        tiffname=strcat(folder,'retardance/vol',num2str(slice_index),'/',coord,'_ret_aip.tif');
        t = Tiff(tiffname,'w');
...
Ret_stitch('ret', folder,disp,mosaic,pxlsize,islice,pattern,sys)
